...
## Copied from https://github.com/godotneers/testing-video
...
func run():
	...
	if _at_start:
		_current_tween.finished.connect(func():
			_is_moving = false
			print("Arrived at start. Is moving? ", _is_moving)
			arrived_at_start.emit()
		)
	else:
		_current_tween.finished.connect(func():
			_is_moving = false
			print("Arrived at end. Is moving? ", _is_moving)
			arrived_at_end.emit()
		)
